...
onPaginationChange(event, value){
  ...
  searchData(search){
    this.state.filteredData = search==""?this.data:  this.data.filter(e=>e.user?.email.includes(search)||e.user?.userName.includes(search)||e.user?.fullName.includes(search))
    this.setState({
    })
  }
    ...
    <div className="col-md-12">
     ...
     <div className="col-md-3">
            <div className="form-group">
              <label asp-for="City">حسب حالة الطلب</label>
              <select
                asp-for="City"
                name="room"
         ...
         <div className="table-responsive">  
            <table className="table filesTable table-striped" id="example"> 
              ...
              <tbody>
                  ...
                  {/* <td>{e?.order?.city}</td> */}
                  {/* <td>{e?.user?.profileImage&&<img  src={window.baseurl+"uploads/"+e.user?.profileImage} alt="img" />}</td> */}
                  <td><h5 className="w-[150px] font-bold text-[18px] text-[green] ">{this.orderStatus[e.order?.status]}</h5></td>
                  <td>
                            <Link
